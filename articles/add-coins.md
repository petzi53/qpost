...
A COinS entry is built from key-value pairs that describe a published
work. The main fields for a blog post are:
...
Additional optional fields can include: - `rft.pub` — Publication/blog
name - `rft.spage`, `rft.epage` — Page numbers (not applicable to blog
posts) - `rft.issn` — Journal ISSN (not applicable to blog posts) -
...
function automates COinS generation. Here’s how to use it:
...
1.  **Reads your post’s YAML front matter** — extracts title, author,
    date, categories, description, etc.
2.  **Queries `_quarto.yml`** — reads the blog’s site URL and language
    settings
3.  **Applies auto-resolution** — fills in missing fields from defaults:
    - `language` — from post YAML → from `_quarto.yml` → from
      `getOption("qpost.lang")` → omitted
    - `license` — from post YAML → from `getOption("qpost.license")` →
      omitted
4.  **Generates the COinS metadata** — creates the properly formatted
    key-value string
5.  **Creates an R code chunk** — appends a chunk to your post that
...
Declares the metadata format as “key-encoded-value” (`kev`) for a
journal-like resource. We use this for blog posts because they’re
closest to journal articles in structure.

    rft.genre=blogPost

Identifies the resource type as a blog post (not a book, journal
...
Language code (optional). Defaults to your blog’s language from
`_quarto.yml`.
...
License identifier (optional). Set in `.Rprofile` to apply the same
license to all posts.
...
[`add_coins()`](https://petzi53.github.io/qpost/reference/add_coins.md)
builds metadata using this priority order:
...
This means: - If your `_quarto.yml` already specifies a language, you
don’t need to configure `qpost.lang` - You only need to set
`qpost.license` once in your `.Rprofile` to apply it to all posts -
