...
Creating a new Quarto blog post usually involves several repetitive
steps: creating a directory with the correct naming convention,
generating an `index.qmd` file with valid YAML front matter, setting
metadata like author and publication date, and organizing categories.

The **qpost** package simplifies this workflow with an interactive
dialog built with Shiny. Instead of manually typing YAML and directory
names, you fill in a form, and
[`qpost()`](https://petzi53.github.io/qpost/reference/qpost.md) handles
the scaffolding automatically. The dialog and the file-opening step rely
on the `rstudioapi` package, so
[`qpost()`](https://petzi53.github.io/qpost/reference/qpost.md) runs in
