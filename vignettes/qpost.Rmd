---
title: "Creating Quarto Blog Posts with qpost"
description: "Learn how to use the qpost() function to interactively scaffold new Quarto blog posts with properly formatted YAML front matter."
...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Creating Quarto Blog Posts with qpost}
...
Creating a new Quarto blog post usually involves several repetitive steps: creating a directory with the correct naming convention, generating an `index.qmd` file with valid YAML front matter, setting metadata like author and publication date, and organizing categories. 

The **qpost** package simplifies this workflow with an interactive dialog built with Shiny. Instead of manually typing YAML and directory names, you fill in a form, and `qpost()` handles the scaffolding automatically. The dialog and the file-opening step rely on the `rstudioapi` package, so `qpost()` runs in **RStudio** and in **Positron** (which ships compatibility shims for the RStudio API), but not in plain VS Code or a terminal R session.
...
`qpost()` is designed for:
- Quarto blog projects with a typical directory structure (`posts/` directory with per-post subdirectories)
- Users who want to avoid manual YAML formatting and directory creation
- Iterative workflows where you create multiple posts

If you're building a single static website or prefer manual control over post structure, `qpost()` may be overkill.

---

## Comparison with `quarto::new_blog_post()`

Quarto itself provides `quarto::new_blog_post()`, a function that creates a new blog post. Here's how `qpost()` improves upon it:

| Feature | `quarto::new_blog_post()` | `qpost()` |
|---------|---------------------------|----------|
...
**Using `quarto::new_blog_post()`**, you must remember the exact argument names and type everything out:
```r
...
**Using `qpost()`**, you fill in the same information through the dialog shown above — see [Basic Usage](#basic-usage) for how to launch it and [The qpost() Dialog](#the-qpost-dialog) for the full list of fields.

---

## Prerequisites

- **RStudio** 1.1 or later, or **Positron** (both provide the `rstudioapi` integration `qpost()` needs to open the dialog viewer and the created file)
...
`qpost()` is not currently supported in plain VS Code or a terminal R session, since neither provides the `rstudioapi` hooks the function depends on.
...
**In RStudio**, you can assign a keyboard shortcut to it for quick access:
1. Go to **Tools > Modify Keyboard Shortcuts**
2. Search for "Create Quarto Post"
3. Click in the **Shortcut** field and press your desired key combination (e.g., `Ctrl+Shift+Alt+Q`)
4. Click **Apply**
...
When you run `qpost()`, you see a form with the following fields:
...
- **Image** — Path to a featured image file (optional)
  ...
  - If selected, the file is copied to the post directory as `image.*` (preserving extension)
  - The YAML front matter includes: `image: image.png` (or `.jpg`, `.svg`, etc.)
...
You can set defaults in your **project-level** `.Rprofile` (in the blog's root directory) to avoid typing the same information repeatedly:
...
**Global defaults** are best set in your personal `~/.Rprofile` and apply to all projects.  
**Project defaults** go in the project's `.Rprofile` in the blog root directory.
...
The `index.qmd` file opens in the editor immediately (in RStudio or Positron), ready for you to start writing.

---

## Verbose Output

If `getOption("qpost.verbose")` is `TRUE` (recommended during setup), `qpost()` prints status messages:
...
`qpost()` searches up from the current working directory to find your Quarto blog's `_quarto.yml` file. If it can't find one, ensure:
- You're opening the RStudio project (or Positron workspace) for your blog (not a parent directory)
- The `_quarto.yml` file exists in the project root
- Your working directory is set to the project root

### "Category dropdown is empty"

`qpost()` reads existing categories from your blog's posts. If you have no posts yet, or all posts have empty category fields, the dropdown will be empty. You can still type new categories in the text field.
...
- Use `add_coins()` to add bibliographic metadata for Zotero compatibility (see the "Adding COinS Metadata for Zotero" vignette)

---

## See Also

- [`add_coins()`](../reference/add_coins.html) — Add COinS metadata to blog posts for Zotero import
